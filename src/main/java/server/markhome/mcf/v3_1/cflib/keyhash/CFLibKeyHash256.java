 ...
 * @author msobkow
 */
public class CFLibKeyHash256 extends CFLibKeyHashBase<CFLibKeyHash256> implements ICFLibKeyHash256, Serializable {

  static final long serialVersionUID = 202608160343L;

  static public int compareOrdered(CFLibKeyHash256 h1, CFLibKeyHash256 h2) {
    if (h1 == null) {
      if (h2 == null) {
        return 0;
      }
  ...
  public static CFLibKeyHash256 fromHex(String string) {
    byte[] b = sbytesFromHex(string);
    CFLibKeyHash256 h = new CFLibKeyHash256();
    h.bytes = b;
    return h;
  }

  public static Comparator<CFLibKeyHash256> getComparator() {

    return new Comparator<CFLibKeyHash256>() {
      @Override
      public int compare(CFLibKeyHash256 a, CFLibKeyHash256 b) {
        return compareOrdered(a, b);
      }
    };
  }

  public CFLibKeyHash256() {
    super();
  }
   ...
   */
  public CFLibKeyHash256(String hexId) {
    super(hexId);
  }

  public CFLibKeyHash256(byte[] anId) {
    super(anId);
  }

  public CFLibKeyHash256(CFLibKeyHash256 otherKey) {
    super(otherKey);
  }

  public CFLibKeyHash256(CFLibKeyHash384 otherKey) {
    super();
    if (otherKey == null) {
  ...
  public CFLibKeyHash256(CFLibKeyHash512 otherKey) {
    super();
    if (otherKey == null) {
      bytes = new byte[HASH_LENGTH];
      return;
  ...
  public static CFLibKeyHash256 fromInt(int v) {
    CFLibKeyHash256 h = nullGet();
  ...
  public CFLibKeyHash256(int notUsed) {
    super(notUsed);
  }

  public static final boolean isNull(CFLibKeyHash256 anId) {
   ...
   */
  static public CFLibKeyHash256 nullGet() {
    CFLibKeyHash256 k = new CFLibKeyHash256();
  ...
  public static CFLibKeyHash256 hash(String text) {
    if (text != null) {
      try {
        ...
        return new CFLibKeyHash256(md.digest());
      }
      catch (Exception ex) {
      }
    }
    return new CFLibKeyHash256(0);
  }

  public static CFLibKeyHash256 hash(byte[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      md.update(payload);
  ...
  public static CFLibKeyHash256 hash(CFLibKeyHash256... payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      for (CFLibKeyHash256 k : payload) {
        md.update(k.bytes);
      }
      return new CFLibKeyHash256(md.digest());
    }
  ...
  public static CFLibKeyHash256 hash(int[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      for (int x : payload) {
  ...
  @Override
  public CFLibKeyHash256 deepClone() {
    return new CFLibKeyHash256(this);
  }

  static public CFLibKeyHash256 fromHexQuick(String string) {
    if (string == null) {
      return null;
    }
  ...
  public static final CFLibKeyHash256[] toCFLibKeyHash256(String[] ids) {
    if (ids == null) {
      return null;
    }
    if (ids.length == 0) {
    ...
    CFLibKeyHash256[] r = new CFLibKeyHash256[ids.length];
    for (int i = 0; i < ids.length; i++) {
      r[i] = new CFLibKeyHash256(ids[i]);
    }
    return r;
  }

  public static final List<CFLibKeyHash256> toCFLibKeyHash256List(String[] ids) {
    ...
    List<CFLibKeyHash256> r = new ArrayList<CFLibKeyHash256>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash256(ids[i]));
    }
    return r;

  }

  public static final Set<CFLibKeyHash256> toCFLibKeyHash256Set(String[] ids) {
    ...
    Set<CFLibKeyHash256> r = new HashSet<CFLibKeyHash256>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash256(ids[i]));
    }
    return r;
