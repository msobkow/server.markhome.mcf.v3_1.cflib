 ...
 * @author msobkow
 */
public class CFLibKeyHash160 extends CFLibKeyHashBase<CFLibKeyHash160> implements ICFLibKeyHash160, Serializable {

  static final long serialVersionUID = 202608160341L;

  static public int compareOrdered(CFLibKeyHash160 h1, CFLibKeyHash160 h2) {
    if (h1 == null) {
      if (h2 == null) {
        return 0;
      }
  ...
  public static CFLibKeyHash160 fromHex(String string) {
    byte[] b = sbytesFromHex(string);
    CFLibKeyHash160 h = new CFLibKeyHash160();
    h.bytes = b;
    return h;
  }

  public static Comparator<CFLibKeyHash160> getComparator() {

    return new Comparator<CFLibKeyHash160>() {
      @Override
      public int compare(CFLibKeyHash160 a, CFLibKeyHash160 b) {
        return compareOrdered(a, b);
      }
    };
  }

  public CFLibKeyHash160() {
    super();
  }
   ...
   */
  public CFLibKeyHash160(String hexId) {
    super(hexId);
  }

  public CFLibKeyHash160(byte[] anId) {
    super(anId);
  }

  public CFLibKeyHash160(CFLibKeyHash160 otherKey) {
    super(otherKey);
  }

  public CFLibKeyHash160(CFLibKeyHash224 k) {
    super();
  ...
  public CFLibKeyHash160(CFLibKeyHash256 k) {
    super();
    bytes = new byte[HASH_LENGTH];
    if (k != null) {
      System.arraycopy(k.getBytes(), 0, bytes, 0, HASH_LENGTH);
  ...
  public CFLibKeyHash160(CFLibKeyHash384 k) {
    super();
    bytes = new byte[HASH_LENGTH];
    if (k != null) {
      System.arraycopy(k.getBytes(), 0, bytes, 0, HASH_LENGTH);
  ...
  public CFLibKeyHash160(CFLibKeyHash512 k) {
    super();
    bytes = new byte[HASH_LENGTH];
    if (k != null) {
      System.arraycopy(k.getBytes(), 0, bytes, 0, HASH_LENGTH);
  ...
  public static CFLibKeyHash160 fromInt(int v) {
    CFLibKeyHash160 h = nullGet();
  ...
  public CFLibKeyHash160(int notUsed) {
    super(notUsed);
  }

  public static final boolean isNull(CFLibKeyHash160 anId) {
   ...
   */
  static public CFLibKeyHash160 nullGet() {
    CFLibKeyHash160 k = new CFLibKeyHash160();
  ...
  public static CFLibKeyHash160 hash(String text) {
    if (text != null) {
      try {
        ...
        return new CFLibKeyHash160(md.digest());
      }
      catch (Exception ex) {
      }
    }
    return new CFLibKeyHash160(0);
  }

  public static CFLibKeyHash160 hash(byte[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      md.update(payload);
  ...
  public static CFLibKeyHash160 hash(CFLibKeyHash160... payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      for (CFLibKeyHash160 k : payload) {
        md.update(k.bytes);
      }
      return new CFLibKeyHash160(md.digest());
    }
  ...
  public static CFLibKeyHash160 hash(int[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      for (int x : payload) {
  ...
  @Override
  public CFLibKeyHash160 deepClone() {
    return new CFLibKeyHash160(this);
  }

  static public CFLibKeyHash160 fromHexQuick(String string) {
    if (string == null) {
      return null;
    }
  ...
  public static final CFLibKeyHash160[] toCFLibKeyHash160(String[] ids) {
    if (ids == null) {
      return null;
    }
    if (ids.length == 0) {
    ...
    CFLibKeyHash160[] r = new CFLibKeyHash160[ids.length];
    for (int i = 0; i < ids.length; i++) {
      r[i] = new CFLibKeyHash160(ids[i]);
    }
    return r;
  }

  public static final List<CFLibKeyHash160> toCFLibKeyHash160List(String[] ids) {
    ...
    List<CFLibKeyHash160> r = new ArrayList<CFLibKeyHash160>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash160(ids[i]));
    }
    return r;

  }

  public static final Set<CFLibKeyHash160> toCFLibKeyHash160Set(String[] ids) {
    ...
    Set<CFLibKeyHash160> r = new HashSet<CFLibKeyHash160>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash160(ids[i]));
    }
    return r;
