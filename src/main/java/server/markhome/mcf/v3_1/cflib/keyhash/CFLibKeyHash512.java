 ...
 * @author msobkow
 */
public class CFLibKeyHash512 extends CFLibKeyHashBase<CFLibKeyHash512> implements ICFLibKeyHash512, Serializable {

  static final long serialVersionUID = 202608160345L;

  static public int compareOrdered(CFLibKeyHash512 h1, CFLibKeyHash512 h2) {
    if (h1 == null) {
      if (h2 == null) {
        return 0;
      }
  ...
  public static CFLibKeyHash512 fromHex(String string) {
    byte[] b = sbytesFromHex(string);
    CFLibKeyHash512 h = new CFLibKeyHash512();
    h.bytes = b;
    return h;
  }

  public static Comparator<CFLibKeyHash512> getComparator() {

    return new Comparator<CFLibKeyHash512>() {
      @Override
      public int compare(CFLibKeyHash512 a, CFLibKeyHash512 b) {
        return compareOrdered(a, b);
      }
    };
  }

  public CFLibKeyHash512() {
    super();
  }
   ...
   */
  public CFLibKeyHash512(String hexId) {
    super(hexId);
  }

  public CFLibKeyHash512(byte[] anId) {
    super(anId);
  }

  public CFLibKeyHash512(CFLibKeyHash512 otherKey) {
    super(otherKey);
  }

  public static CFLibKeyHash512 fromInt(int v) {
    CFLibKeyHash512 h = nullGet();
  ...
  public CFLibKeyHash512(int notUsed) {
    super(notUsed);
  }

  public static final boolean isNull(CFLibKeyHash512 anId) {
   ...
   */
  static public CFLibKeyHash512 nullGet() {
    CFLibKeyHash512 k = new CFLibKeyHash512();
  ...
  public static CFLibKeyHash512 hash(String text) {
    if (text != null) {
      try {
        ...
        return new CFLibKeyHash512(md.digest());
      }
      catch (Exception ex) {
      }
    }
    return new CFLibKeyHash512(0);
  }

  public static CFLibKeyHash512 hash(byte[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      md.update(payload);
  ...
  public static CFLibKeyHash512 hash(CFLibKeyHash512... payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      for (CFLibKeyHash512 k : payload) {
        md.update(k.bytes);
      }
      return new CFLibKeyHash512(md.digest());
    }
  ...
  public static CFLibKeyHash512 hash(int[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      for (int x : payload) {
  ...
  @Override
  public CFLibKeyHash512 deepClone() {
    return new CFLibKeyHash512(this);
  }

  static public CFLibKeyHash512 fromHexQuick(String string) {
    if (string == null) {
      return null;
    }
  ...
  public static final CFLibKeyHash512[] toCFLibKeyHash512(String[] ids) {
    if (ids == null) {
      return null;
    }
    if (ids.length == 0) {
    ...
    CFLibKeyHash512[] r = new CFLibKeyHash512[ids.length];
    for (int i = 0; i < ids.length; i++) {
      r[i] = new CFLibKeyHash512(ids[i]);
    }
    return r;
  }

  public static final List<CFLibKeyHash512> toCFLibKeyHash512List(String[] ids) {
    ...
    List<CFLibKeyHash512> r = new ArrayList<CFLibKeyHash512>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash512(ids[i]));
    }
    return r;

  }

  public static final Set<CFLibKeyHash512> toCFLibKeyHash512Set(String[] ids) {
    ...
    Set<CFLibKeyHash512> r = new HashSet<CFLibKeyHash512>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash512(ids[i]));
    }
    return r;
