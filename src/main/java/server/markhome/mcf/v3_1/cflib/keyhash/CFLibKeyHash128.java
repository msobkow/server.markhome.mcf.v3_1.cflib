...
public class CFLibKeyHash128 extends CFLibKeyHashBase<CFLibKeyHash128> implements ICFLibKeyHash128, Serializable {
  ...
  public static CFLibKeyHash128 fromHex(String string) {
    byte[] b = sbytesFromHex(string);
    CFLibKeyHash128 h = new CFLibKeyHash128();
    h.bytes = b;
    return h;
  }
  ...
  public CFLibKeyHash128() {
  }

  /**
   * This is the hex code of the underlying ID. THIS IS NOT A HASHING FUNCTION.
   */
  public CFLibKeyHash128(String hexId) {
    super(hexId);
  }

  public CFLibKeyHash128(byte[] anId) {
    super(anId);
  }
  ...
  public static CFLibKeyHash128 fromInt(int v) {
    CFLibKeyHash128 h = nullGet();
  ...
  public CFLibKeyHash128(int notUsed) {
    super(notUsed);
  }

  public static final boolean isNull(CFLibKeyHash128 anId) {
  ...
  public static CFLibKeyHash128 hash(String text) {
    if (text != null) {
      try {
  ...
  public static CFLibKeyHash128 hash(byte[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      md.update(payload);
  ...
  public static CFLibKeyHash128 hash(int[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      for (int x : payload) {
  ...
  @Override
  public CFLibKeyHash128 deepClone() {
    return new CFLibKeyHash128(this);
  }

  static public CFLibKeyHash128 fromHexQuick(String string) {
    if (string == null) {
      return null;
    }
  ...
  public static final CFLibKeyHash128[] toCFLibKeyHash128(String[] ids) {
    if (ids == null) {
      return null;
    }
    if (ids.length == 0) {
    ...
    CFLibKeyHash128[] r = new CFLibKeyHash128[ids.length];
    for (int i = 0; i < ids.length; i++) {
      r[i] = new CFLibKeyHash128(ids[i]);
    }
    return r;
  }

  public static final List<CFLibKeyHash128> toCFLibKeyHash128List(String[] ids) {
    ...
    List<CFLibKeyHash128> r = new ArrayList<CFLibKeyHash128>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash128(ids[i]));
    }
    return r;

  }

  public static final Set<CFLibKeyHash128> toCFLibKeyHash128Set(String[] ids) {
    ...
    Set<CFLibKeyHash128> r = new HashSet<CFLibKeyHash128>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash128(ids[i]));
    }
    return r;
