 ...
 * implementations, such as CFLibKeyHash256 or CFLibKeyHash512. It also provides a consistent way to handle the underlying byte arrays, ensuring that all
 ...
 * @author msobkow
 */
public abstract class CFLibKeyHashBase<T extends CFLibKeyHashBase<T>> implements Comparator<T>, Comparable<T> {
	...
	public CFLibKeyHashBase(T otherKey) {
		if (otherKey == null) {
			setBytes(new byte[getHashLength()]);
			return;
		}
	...
	public CFLibKeyHashBase(int notUsed) {
		initStatics();
