 ...
 * @author msobkow
 */
public class CFLibKeyHash384 extends CFLibKeyHashBase<CFLibKeyHash384> implements ICFLibKeyHash384, Serializable {
  static final long serialVersionUID = 202608160344L;
  protected byte[] bytes;

  @Override
  @SuppressWarnings("unchecked")
  public int compareTo(ICFLibKeyHash384 o) {
    int result = compare((ICFLibKeyHash384) this, o);
    return result;
  }

  static public int compareOrdered(ICFLibKeyHash384 h1, ICFLibKeyHash384 h2) {
    if (h1 == null) {
      if (h2 == null) {
        return 0;
      }
  ...
  public static CFLibKeyHash384 fromHex(String string) {
    byte[] b = sbytesFromHex(string);
    CFLibKeyHash384 h = new CFLibKeyHash384();
    h.bytes = b;
    return h;
  }

  public static Comparator<ICFLibKeyHash384> getComparator() {

    return new Comparator<ICFLibKeyHash384>() {
      @Override
      public int compare(ICFLibKeyHash384 a, ICFLibKeyHash384 b) {
        return compareOrdered(a, b);
      }
    };
  }

  public CFLibKeyHash384() {
    super();
  }
   ...
   */
  public CFLibKeyHash384(String hexId) {
    super(hexId);
  }

  public CFLibKeyHash384(byte[] anId) {
    super(anId);
  }

  public CFLibKeyHash384(ICFLibKeyHash384 otherKey) {
  ...
  public CFLibKeyHash384(ICFLibKeyHash512 otherKey) {
    super();
    if (otherKey == null) {
      bytes = new byte[HASH_LENGTH];
      return;
  ...
  public static CFLibKeyHash384 fromInt(int v) {
    CFLibKeyHash384 h = nullGet();
  ...
  public CFLibKeyHash384(int notUsed) {
    super(notUsed);
  }

  public static final boolean isNull(CFLibKeyHash384 anId) {
   ...
   */
  static public CFLibKeyHash384 nullGet() {
    CFLibKeyHash384 k = new CFLibKeyHash384(new byte[HASH_LENGTH]);
    return k;
  }
  ...
  public static CFLibKeyHash384 hash(String text) {
    if (text != null) {
      try {
        ...
        return new CFLibKeyHash384(md.digest());
      }
      catch (Exception ex) {
      }
    }
    return new CFLibKeyHash384(0);
  }

  public static CFLibKeyHash384 hash(byte[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      md.update(payload);
  ...
  public static CFLibKeyHash384 hash(ICFLibKeyHash384... payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      for (ICFLibKeyHash384 k : payload) {
  ...
  public static CFLibKeyHash384 hash(int[] payload) {
    try {
      MessageDigest md = MessageDigest.getInstance(HASH_ALGO);
      for (int x : payload) {
  ...
  @Override
  public CFLibKeyHash384 deepClone() {
    return new CFLibKeyHash384(this);
  }

  static public CFLibKeyHash384 fromHexQuick(String string) {
    if (string == null) {
      return null;
    }
  ...
  public static final CFLibKeyHash384[] toCFLibKeyHash384(String[] ids) {
    if (ids == null) {
      return null;
    }
    if (ids.length == 0) {
    ...
    CFLibKeyHash384[] r = new CFLibKeyHash384[ids.length];
    for (int i = 0; i < ids.length; i++) {
      r[i] = new CFLibKeyHash384(ids[i]);
    }
    return r;
  }

  public static final List<CFLibKeyHash384> toCFLibKeyHash384List(String[] ids) {
    ...
    List<CFLibKeyHash384> r = new ArrayList<CFLibKeyHash384>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash384(ids[i]));
    }
    return r;

  }

  public static final Set<CFLibKeyHash384> toCFLibKeyHash384Set(String[] ids) {
    ...
    Set<CFLibKeyHash384> r = new HashSet<CFLibKeyHash384>(ids.length);
    for (int i = 0; i < ids.length; i++) {
      r.add(new CFLibKeyHash384(ids[i]));
    }
    return r;
